...
def remove_overlap3(plags, psr, src_bow, susp_bow):
    ...
    if len(plags) != 0:
        plags, psr = list(map(list, list(zip(*sorted(zip(plags, psr), key = lambda tup: tup[0][0][0])))))
    res_plags = []
    res_psr = []
    flag = 0
    i = 0
    while i < len(plags):
        ...
        if cont_ol == 0:
            ...
            for j in range(1, cont_ol + 1):
                if flag == 0:
                    sents_i = list(range(plags[i][0][0], plags[i][0][1] + 1))
                    range_i = list(range(plags[i][1][0], plags[i][1][1] + 1))
                else:
                    sents_i = list(range(res_plags[-1][0][0], res_plags[-1][0][1] + 1))
                    range_i = list(range(res_plags[-1][1][0], res_plags[-1][1][1] + 1))
                sents_j = list(range(plags[i + j][0][0], plags[i + j][0][1] + 1))
                sim_i_ol = 0.0
                sim_j_ol = 0.0
                sim_i_nol = 0.0
                sim_j_nol = 0.0
                ...
                for sent in sents_i:
                    ...
                    if sent in sents_j:
                        ...
                        sim_i_nol += sim_max
                        cont_i_nol_sents += 1
                range_j = list(range(plags[i + j][1][0], plags[i + j][1][1] + 1))
                for sent in sents_j:
                    sim_max = 0.0
                    for k in range_j:
                        sim = cosine_measure(susp_bow[sent], src_bow[k])
